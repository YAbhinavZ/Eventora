    ...
    const fetchEvent = async () => {
      try {
        ...
        if (user) {
          ...
          const myBooking = bookingsRes.data.find((b) => b.eventId?._id === id);
          if (myBooking) {
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 min-w-[300px] w-full md:w-auto shrink-0 shadow-sm">
            ...
            {alreadyBooked ? (
              bookingStatus === "pending" ? (
                <div className="w-full py-4 text-center bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-xl font-bold">
                  ⏳ Booking Pending Admin Approval
                </div>
              ) : (
                <div className="w-full py-4 text-center bg-green-50 text-green-700 border border-green-200 rounded-xl font-bold">
                  ✓ Booking Confirmed
                </div>
              )
            ) : (
